...
class Gui:
    def __init__(self):
        ...
        self.__surface_image = np.zeros((*INITIAL_WINDOW_SIZE, 3), np.uint8)
    ...
    def __render(self):
        ...
        if self.__image is not None:
            image_width, image_height, _ = np.shape(self.__image)
        ...
        for (align_h, align_v), texts in self.__texts_map.items():
            ...
            for s in texts:
                (text_width, text_height), _ = cv2.getTextSize(
                    s, cv2.FONT_HERSHEY_PLAIN, 1.0, 1
                )
                cv2.putText(
                    self.__surface_image,
                    s,
                    (
                        int((window_width - text_width) * h_factor),
                        int((window_height - text_height) * v_factor) + offset_y,
                    ),
        ...
        pg.surfarray.blit_array(self.__surface, self.__surface_image)
        pg.display.flip()
